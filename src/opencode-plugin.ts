...
async function captureSession(
  ...
  try {
    ...
    const messages = extract<SessionMessage[]>(out) ?? [];
...
async function summarize(
  ...
  try {
    ...
    sid = (created as { id?: string }).id ?? (created as { data?: { id?: string } }).data?.id ?? "";
    if (!sid) return null;
    busy.add(sid);
    ...
    let parsed: StructuredSummary | null = info?.structured_output ?? null;
    if (!parsed) {
      const text = textOf((payload as { parts?: PartLike[] }).parts ?? []);
...
async function recentHighlights($: PluginInput["$"]): Promise<string> {
  try {
    const out = await $`mm ls --json --limit 3`.quiet().nothrow();
    const parsed = JSON.parse(out.stdout.toString());
    if (!Array.isArray(parsed)) return "";
    return parsed.map((m: { title?: string; id?: string }) => `- ${m.title ?? "?"} (${m.id ?? ""})`).join("\n");
  } catch {
    return "";
  }
}

async function runMnemo($: PluginInput["$"], args: string[]): Promise<void> {
  await $`mm ${args}`.quiet().nothrow();
}

function extract<T>(value: unknown): T | null {
  if (value === undefined || value === null) return null;
  const v = value as { data?: T };
  return v.data ?? (value as T);
}
...
function extractJson(text: string): StructuredSummary | null {
  ...
  try {
    return JSON.parse(candidate) as StructuredSummary;
  } catch {
    return null;
  }
}
