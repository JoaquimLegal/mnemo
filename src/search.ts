...
export function docText(memory: Memory): string {
  return `${memory.title} ${memory.title} ${memory.body} ${memory.tags.join(" ")}`;
}

export function bm25Scores(memories: Memory[], query: string): Map<string, number> {
  ...
  for (let i = 0; i < N; i++) {
    ...
    for (const t of tokens[i]) freq.set(t, (freq.get(t) ?? 0) + 1);
    const len = tokens[i].length;
    let s = 0;
    for (const q of qTerms) {
      if (!freq.has(q) || !df.has(q)) continue;
...
export function search(
  ...
  for (const m of memories) {
    ...
    if (requireMatch && !bm.has(m.id)) continue;
