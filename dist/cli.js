...
async function main() {
    ...
    switch (cmd) {
        ...
        case "ls": {
            ...
            let memories = (await store.list(values.agent)).filter((m) => wanted.length === 0 || wanted.every((t) => m.tags.includes(t)));
            if (limit)
        ...
        case "search": {
            ...
            if (values.json) {
                console.log(JSON.stringify(results.map((r) => ({ ...r.memory, score: r.score })), null, 2));
                return;
            }
            for (const r of results) {
                const { memory } = r;
                console.log(`${memory.id}  ${r.score.toFixed(3)}  [${memory.agent}]  ${memory.title}`);
            }
            if (results.length === 0)
        ...
        case "log": {
            ...
            for (const e of entries)
                console.log(`${e.hash}  ${e.date}  ${e.message}`);
            if (entries.length === 0)
...
async function setupOpenCode() {
    ...
    const pluginExists = await fs
        ...
        .catch(() => false);
    if (!pluginExists) {
        console.error(`plugin not found at ${pluginSource} — run "npm run build" first or reinstall mnemo`);
        process.exit(1);
    }
